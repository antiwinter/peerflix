...
var argv = rc(
  ...
  optimist
    ...
    .default('p', 8888)
    .alias('i', 'index')
    .describe('i', 'changed streamed file (index)')
    .alias('l', 'list')
    .describe('l', 'list available files with corresponding index')
    .boolean('l')
    .alias('t', 'subtitles')
    .describe('t', 'load subtitles file')
    .alias('q', 'quiet')
    .describe('q', 'be quiet')
    .boolean('v')
    .alias('v', 'vlc')
    .describe('v', 'autoplay in vlc*')
    .boolean('v')
    .alias('s', 'airplay')
    .describe('s', 'autoplay via AirPlay')
    .boolean('a')
    .alias('z', 'iina')
    .describe('z', 'autoplay in iina')
    .boolean('z')
    .alias('m', 'mplayer')
    .describe('m', 'autoplay in mplayer*')
    .boolean('m')
    .alias('g', 'smplayer')
    .describe('g', 'autoplay in smplayer*')
    .boolean('g')
    .describe('mpchc', 'autoplay in MPC-HC player*')
    .boolean('boolean')
    .describe('potplayer', 'autoplay in Potplayer*')
    .boolean('boolean')
    .alias('k', 'mpv')
    .describe('k', 'autoplay in mpv*')
    .boolean('k')
    .alias('o', 'omx')
    .describe('o', 'autoplay in omx**')
    .boolean('o')
    .alias('w', 'webplay')
    .describe('w', 'autoplay in webplay')
    .boolean('w')
    .alias('j', 'jack')
    .describe('j', 'autoplay in omx** using the audio jack')
    .boolean('j')
    .alias('f', 'path')
    .describe('f', 'change buffer file path')
    .alias('b', 'blocklist')
    .describe('b', 'use the specified blocklist')
    .alias('n', 'no-quit')
    .describe('n', 'do not quit peerflix on vlc exit')
    .boolean('n')
    .alias('a', 'all')
    .describe('a', 'select all files in the torrent')
    .boolean('a')
    .alias('r', 'remove')
    .describe('r', 'remove files on exit')
    .boolean('r')
    .alias('h', 'hostname')
    .describe('h', 'host name or IP to bind the server to')
    .alias('e', 'peer')
    .describe('e', 'add peer by ip:port')
    .alias('x', 'peer-port')
    .describe('x', 'set peer listening port')
    .alias('d', 'not-on-top')
    .describe('d', 'do not float video on top')
    .boolean('d')
...
var MPLAYER_EXEC =
  'mplayer ' + (onTop ? '-ontop' : '') + ' -really-quiet -noidx -loop 0 '
...
if (argv.t) {
  VLC_ARGS +=
    ' --sub-file=' + (process.platform === 'win32' ? argv.t : enc(argv.t))
...
var watchVerifying = function(engine) {
  var showVerifying = function(i) {
    var percentage = Math.round(
      ((i + 1) / engine.torrent.pieces.length) * 100.0
    )
    clivas.clear()
...
var ontorrent = function(torrent) {
  ...
  engine.server.on('listening', function() {
    ...
    var filename = engine.server.index.name
      .split('/')
      .pop()
      .replace(/\{|\}/g, '')
    ...
    if (argv.vlc && process.platform === 'win32') {
      ...
      registry('HKLM', '\\Software\\VideoLAN\\VLC', 'InstallDir', function(
        err,
        regItem
      ) {
        if (err) {
      ...
      player = 'mph-hc'
      registry('HKCU', '\\Software\\MPC-HC\\MPC-HC', 'ExePath', function(
        err,
        regItem
      ) {
        if (err) return
      ...
      var runPotPlayer = function(regItem) {
        proc.exec(
          '"' + regItem.value + '" "' + localHref + '" ' + POTPLAYER_ARGS
        )
      }
      registry('HKCU', '\\Software\\DAUM\\PotPlayer64', 'ProgramPath', function(
        err,
        regItem
      ) {
        if (err) {
    ...
    var draw = function() {
      ...
      var runtime = Math.floor(
        (Date.now() - started - timePaused - timeCurrentPause) / 1000
      )
      ...
      if (argv.airplay) {
        if (airplayServer)
          clivas.line(
            '{green:streaming to} {bold:' +
              airplayServer.name +
              '} {green:using airplay}'
          )
        else clivas.line('{green:streaming} {green:using airplay}')
      } else {
        clivas.line('{bold:' + href + '}')
      }
      clivas.line('')
      clivas.line(
        '{yellow:info} {green:streaming} {bold:' +
          filename +
          ' (' +
          bytes(filelength) +
          ')} {green:-} {bold:' +
      ...
      clivas.line(
        '{yellow:info} {green:downloaded} {bold:' +
          bytes(swarm.downloaded) +
          '} (' +
          downloadedPercentage +
          '%) {green:and uploaded }{bold:' +
          bytes(swarm.uploaded) +
          '} {green:in }{bold:' +
          runtime +
          's} {green:with} {bold:' +
          hotswaps +
      ...
      clivas.line(
        '{yellow:info} {green:verified} {bold:' +
          verified +
          '} {green:pieces and received} {bold:' +
          invalid +
          '} {green:invalid pieces}'
      )
      clivas.line(
        '{yellow:info} {green:peer queue size is} {bold:' + swarm.queued + '}'
      )
      clivas.line('{80:}')
      ...
      if (interactive) {
        ...
        if (paused)
          clivas.line(
            '{yellow:PAUSED} {green:Press SPACE to continue download' + openLoc
          )
  ...
  var onmagnet = function() {
    clivas.clear()
    clivas.line(
      '{green:fetching torrent metadata from} {bold:' +
